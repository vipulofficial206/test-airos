...
class AirOSDesktopApp:
    ...
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("AirOS++ Control Center | Spatial HCI Framework")
    ...
    def _build_ui(self):
        ...
        title_lbl = ttk.Label(top_bar, text="AirOS++ Spatial HCI Framework", style="Header.TLabel")
    ...
    def _build_tab_controls(self, parent):
        ...
        box_mode.pack(fill=tk.X, padx=10, pady=10)
        ...
        chk_dry.grid(row=1, column=0, columnspan=2, sticky=tk.W, padx=10, pady=5)
        ...
        chk_hud.grid(row=2, column=0, columnspan=2, sticky=tk.W, padx=10, pady=5)

        # Quick Actions
        box_actions = ttk.LabelFrame(parent, text="Quick Gesture Actions Simulator")
        box_actions.pack(fill=tk.X, padx=10, pady=10)

        btn_left_click = ttk.Button(box_actions, text="Trigger Left Click", command=lambda: self._simulate_click("Left Click"))
        btn_left_click.pack(side=tk.LEFT, padx=10, pady=10, expand=True)

        btn_right_click = ttk.Button(box_actions, text="Trigger Right Click", command=lambda: self._simulate_click("Right Click"))
        btn_right_click.pack(side=tk.LEFT, padx=10, pady=10, expand=True)

    def _build_tab_params(self, parent):
        # AMS Tuning
        box_ams = ttk.LabelFrame(parent, text="Algorithm 1: Adaptive Motion Smoothing (AMS)")
        box_ams.pack(fill=tk.X, padx=10, pady=8)

        ttk.Label(box_ams, text="Alpha Min (Stationary Smoothing):").grid(row=0, column=0, sticky=tk.W, padx=5, pady=5)
        ...
        self.slider_alpha_min.grid(row=0, column=1, sticky=tk.EW, padx=5, pady=5)

        ttk.Label(box_ams, text="Alpha Max (Rapid Motion Response):").grid(row=1, column=0, sticky=tk.W, padx=5, pady=5)
        ...
        self.slider_alpha_max.grid(row=1, column=1, sticky=tk.EW, padx=5, pady=5)

        # BSI Tuning
        box_bsi = ttk.LabelFrame(parent, text="Algorithm 2: Bounding Box Stability Index (BSI)")
        box_bsi.pack(fill=tk.X, padx=10, pady=8)

        ttk.Label(box_bsi, text="Stability Threshold Cutoff (T_BSI):").grid(row=0, column=0, sticky=tk.W, padx=5, pady=5)
        ...
        self.slider_bsi_thresh.grid(row=0, column=1, sticky=tk.EW, padx=5, pady=5)

        # ICV Tuning
        box_icv = ttk.LabelFrame(parent, text="Algorithm 3: Intent-Based Click Verification (ICV)")
        box_icv.pack(fill=tk.X, padx=10, pady=8)

        ttk.Label(box_icv, text="Aspect Ratio Shift (Delta AR):").grid(row=0, column=0, sticky=tk.W, padx=5, pady=5)
        ...
        self.slider_ar_shift.grid(row=0, column=1, sticky=tk.EW, padx=5, pady=5)
    ...
    def _build_tab_diag(self, parent):
        ...
        btn_tests = ttk.Button(box_run, text="🧪 Run Pytest Test Suite (37 Tests)", command=self._run_tests_gui)
    ...
    def _build_tab_logs(self, parent):
        self.txt_logs = tk.Text(parent, bg="#11111b", fg="#cdd6f4", font=("Consolas", 9), wrap=tk.WORD)
    ...
    def _simulate_click(self, click_type: str):
        self.txt_logs.insert(tk.END, f"[{time.strftime('%H:%M:%S')}] Simulated Action: {click_type}\n")
    ...
    def _pipeline_worker_loop(self):
        ...
        while self.is_running and self.camera_reader:
            ...
            # Convert BGR frame to RGB ImageTk for Tkinter Display
            rgb_frame = cv2.cvtColor(rendered_frame, cv2.COLOR_BGR2RGB)
            img_pil = Image.fromarray(rgb_frame)

            # Resize to fit UI frame
